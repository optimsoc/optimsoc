...
\path{/opt/toolchains/or1k-elf-multicore}. In case you need to use
...
\path{/opt/toolchains/or1k-elf-multicore}, but the toolchain name (used for
...
found at \path{src/sw/gzll}. Go there and run
...
After you installed the libraries you need to enable them in your
...
\url{https://github.com/optimsoc/host} (repo tree \path{src/host}) in
...
downloaded it and unpacked it to \path{/opt/ztex} so that the folder
\path{/opt/ztex/java} exists. You will need to set the path
accordingly:
...
In the following the installation to \path{/opt} is assumed, change
...
SystemC (we use \path{/opt/systemc} here).
...
You will now find SystemC in \path{/opt/systemc} and need to set
...
This will install Verilator to \path{/usr/local}. Your system should usually
...
\url{https://github.com/optimsoc/systemc} or in \path{src/systemc}
