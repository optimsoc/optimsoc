...
extern "C" {
  ...
  // It can only hold one message
  struct endpoint {
    volatile unsigned int buffer[MCAPI_MAX_MSG_SIZE+3];
    volatile unsigned int message_size;
...
// TODO: if you add more, you have to adjust LSB
...
#define SET(x,v,msb,lsb) (((~0 << (msb+1) | ~(~0 << lsb))&x) | ((v & ~(~0<<(msb-lsb+1))) << lsb))
    ...
    case CTRL_REQUEST_MSG_DATA:
      {
        ...
        for (int i=0;i<size;i++) {
            ep->buffer[i] = buffer[i+3];
        }

        ep->message_size = size;
  ...
  {
    // If any port can be selected, select next number
    // TODO: This only works until endpoints can be deleted
    if (port_num == MCAPI_PORT_ANY) {
        port_num = mcapi_trans_num_endpoints();
    }
      ...
      mca_status_t* mcapi_status)
  {
    ...
    *mcapi_status = MCAPI_ERR_GENERAL; // Not implemented
  }
...
RETURN VALUE: MCAPI_TRUE/MCAPI_FALSE indicating success or failure
      ...
      const char* buffer, size_t buffer_size)
  {
    ...
    ctrl_request.buffer[1] = (unsigned int) recvep->ep;
    ctrl_request.buffer[2] = buffer_size>>2;
    for (int i=0;i<buffer_size>>2;i++) {
        ctrl_request.buffer[3+i] = ((unsigned int *)buffer)[i];
    }

    optimsoc_mp_simple_send(3+buffer_size,ctrl_request.buffer);
  ...
  mcapi_boolean_t mcapi_trans_msg_recv( mcapi_endpoint_t  receive_endpoint,  char* buffer, size_t buffer_size, size_t* received_size)
  {
    ...
    while((ep->flags & EP_FLAG_MSG)==0) { __asm__ volatile("l.nop 0xff"); }

    size = ep->message_size;
